package cc.shencai.commonlibrary.ncalendar.view;
...
public class MonthView extends CalendarView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mRowNum; i++) {
            for (int j = 0; j < 7; j++) {
                ...
                //当月和上下月的颜色不同
                if (Utils.isEqualsMonth(dateTime, mInitialDateTime)) {//当月
                    //当天和选中的日期不绘制农历
    ...
    public void drawPoint(Canvas canvas, Rect rect, DateTime dateTime, int baseline) {
		String a = dateTime.toLocalDate().toString();
        if (hashMapPoint != null && hashMapPoint.containsKey(dateTime.toLocalDate().toString())) {
            mLunarPaint.setColor(hashMapPoint.get(dateTime.toLocalDate().toString()));
