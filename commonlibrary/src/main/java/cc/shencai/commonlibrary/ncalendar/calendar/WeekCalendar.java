...
import cc.shencai.commonlibrary.ncalendar.adapter.CalendarAdapter;
import cc.shencai.commonlibrary.ncalendar.adapter.WeekAdapter;
import cc.shencai.commonlibrary.ncalendar.listener.OnClickWeekViewListener;
...
import cc.shencai.commonlibrary.ncalendar.utils.Utils;
import cc.shencai.commonlibrary.ncalendar.view.CalendarView;
import cc.shencai.commonlibrary.ncalendar.view.WeekView;
...
public class WeekCalendar extends CalendarPager implements OnClickWeekViewListener {
    ...
    protected void initCurrentCalendarView(int position) {
        ...
        //只处理翻页
        if (lastPosition == -1) {
            currView.setDateTimeAndPoint(mInitialDateTime, hashMapPoint);
            ...
            if (isDefaultSelect) {
                ...
                currView.setDateTimeAndPoint(mSelectDateTime, hashMapPoint);
                if (onWeekCalendarChangedListener != null) {
                    onWeekCalendarChangedListener.onWeekCalendarChanged(mSelectDateTime);
                }
            } else {
                if (Utils.isEqualsMonth(lastSelectDateTime, mSelectDateTime)) {
                    currView.setDateTimeAndPoint(lastSelectDateTime, hashMapPoint);
                }
            }
    ...
    protected void setDateTime(DateTime dateTime) {
        ...
        currectWeekView.setDateTimeAndPoint(dateTime, hashMapPoint);
    ...
    public void onClickCurrentWeek(DateTime dateTime) {
        ...
        weekView.setDateTimeAndPoint(dateTime, hashMapPoint);
        mSelectDateTime = dateTime;
        lastSelectDateTime = dateTime;
        if (onWeekCalendarChangedListener != null) {
