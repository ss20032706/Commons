...
public abstract class CalendarPager extends ViewPager {
	...
	protected HashMap<String, Integer> hashMapPoint = new HashMap<>();//圆点
	...
	public CalendarPager(Context context, AttributeSet attrs) {
		...
		Attrs.firstDayOfWeek = "Monday".equals(firstDayOfWeek) ? NCalendar.MONDAY : NCalendar.SUNDAY;
	...
	public synchronized void  setPointList(HashMap<String, Integer> hashMapPoint) {
		HashMap<String, Integer> hashMapPointFormat = new HashMap<>();
		Iterator<Map.Entry<String, Integer>> iter = hashMapPoint.entrySet().iterator();
		while (iter.hasNext()) {
			Map.Entry<String, Integer> entry = iter.next();
			String format = new DateTime(entry.getKey()).toString("yyyy-MM-dd");
			hashMapPointFormat.put(format, entry.getValue());
		}
		...
		this.hashMapPoint.putAll(hashMapPointFormat);
		calendarView.setPointList(hashMapPointFormat);
