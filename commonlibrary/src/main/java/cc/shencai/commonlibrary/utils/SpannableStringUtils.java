...
public class SpannableStringUtils {
	...
	/**
	 * 高亮显示服务器下发的带html标记的字符
	 *
	 * @param color 高亮显示的字体颜色
	 * @param str   要被截取处理的字符串
	 * @return SpannableString类型的结果，可直接设置给TextView
	 */
	public static SpannableString splitString(int color, String str) {
		...
		return matcherSearchTitle(color, allStr, tagList);
	}
	 ...
	 * @param color     变化的色值
	 * @param text      文字
	 * @param arrayList 文字中的关键字集合
	 * @return
	 */
	private static SpannableString matcherSearchTitle(int color, String text,
													  List<String> arrayList) {
		SpannableString s = new SpannableString(text);
		for (int i = 0; i < arrayList.size(); i++) {
			Pattern p = Pattern.compile(arrayList.get(i));
	 ...
	 * @return
	 */
	//Html.fromHtml(number.replace(filterNum, "<font color='#2885C8'>" + filterNum + "</font>"))
	public static Spanned setTextColorWithFiflter(String color, String fiflter, String content){
		return  Html.fromHtml(content.replace(fiflter, "<font color='"+color+"'>" + fiflter +"</font>" ));
	}
	...
	public static class Builder {
		 ...
		 * @return {@link Builder}
		 */
		public Builder appendLine(@NonNull CharSequence text,Context context) {
			return append(text + LINE_SEPARATOR,context);
		}
		...
		private void setSpan(Context context) {
			...
			if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
				if (imageIsBitmap) {
					mBuilder.setSpan(new CustomImageSpan(context, bitmap, align), start, end, flag);
					bitmap = null;
					imageIsBitmap = false;
					...
					mBuilder.setSpan(new CustomImageSpan(context, uri, align), start, end, flag);
					uri = null;
					imageIsUri = false;
				} else {
					mBuilder.setSpan(new CustomImageSpan(context, resourceId, align), start, end, flag);
					resourceId = 0;
