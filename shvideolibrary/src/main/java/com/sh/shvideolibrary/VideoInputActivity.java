...
//        return  new File(getContext().getExternalCacheDir().getAbsolutePath() + "/" + fileName);
	...
	public void checkPermission() {
		if (Build.VERSION.SDK_INT > 22) {
			int permission = ContextCompat.checkSelfPermission(VideoInputActivity.this, Manifest.permission.CAMERA);
			if (permission != PackageManager.PERMISSION_GRANTED) {
				if (shouldRequest()) {
					return;
				} else {
					ActivityCompat.requestPermissions(VideoInputActivity.this, new String[]{Manifest.permission.CAMERA}, 1);
				}
			}
		}
	}
